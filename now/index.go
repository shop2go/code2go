...
func Handler(w http.ResponseWriter, r *http.Request) {
  ...
  <form class="form-inline" role="form" method="get">
					` + strconv.Itoa(c.Year) + ` -> <button class="btn btn-link" onclick="window.location.href='prev'">` + now.AddDate(0, -1, 0).Month().String() + `</button>` + now.Month().String() + `<button type="button" class="btn btn-link" onclick="window.location.href='next'">` + now.AddDate(0, +1, 0).Month().String() + `</button>
					</form>
					</div>
