  ...
  <form class="form-inline" role="form" method="get">
	...
	for k := q; k < 32; k++ {
		...
		case "Tuesday":
			...
			<div class="col-sm">
			` + c.Days[k] + `<br><button type="button" class="btn btn-link" onclick="window.location.href='entry#` + c.Days[k] + `-` + strconv.Itoa(c.Month) + `-` + strconv.Itoa(k) + `'">` + strconv.Itoa(k) + `<span class="badge badge-pill badge-light">` + strconv.Itoa(k) + `</span></button>
				</div> </span>
