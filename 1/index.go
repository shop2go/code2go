...
func Handler(w http.ResponseWriter, r *http.Request) {
  ...
  <button type="button" class="btn btn-link" onclick="window.location.href='` + b + `'">` + now.AddDate(0, n-1, 0).Month().String() + `</button><br>` + strconv.Itoa(c.Year) + `/` + now.Month().String() + `<button type="button" class="btn btn-link" onclick="window.location.href='` + o + `'">` + now.AddDate(0, n+1, 0).Month().String() + `</button></form>
					</div>
